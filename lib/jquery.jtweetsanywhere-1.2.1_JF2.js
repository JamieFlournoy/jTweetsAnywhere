...
(function($)
{
	$.fn.jTweetsAnywhere = function(callerOptions)
	{
      ...
      list_api_hostname: 'api.twitter.com',
      search_api_hostname: 'search.twitter.com',
      timeline_api_hostname: 'api.twitter.com',
      ratelimit_api_hostname: 'api.twitter.com',
			...
			tweetReplyToDecorator: defaultTweetReplyToDecorator,
		...
		if (!options._tweetFeedConfig.showReplyTo)
		{
			options.tweetReplyToDecorator = null;
		}
	...
	defaultTweetAttributesDecorator = function(tweet, options)
	{
		...
		)
		{
			...
			if (options.tweetReplyToDecorator)
			{
				html += options.tweetReplyToDecorator(tweet, options);
			}
	...
	defaultTweetTimestampFormatter = function(timeStamp)
	{
		...
		var diff = parseInt((now.getTime() - Date.parse(timeStamp)) / 1000, 10);
		...
		{
			t = parseInt((diff + 30) / 60, 10);
			tweetTimestamp += t + ' minute' + (t == 1 ? '' : 's') + ' ago';
		}
		else if (diff < 86400)
		{
			t = parseInt((diff + 1800) / 3600, 10);
		...
		else
		{
			...
			t = parseInt((diff + 43200) / 86400, 10);
	...
	{
		var diff = parseInt((new Date().getTime() - Date.parse(timeStamp)) / 1000, 10);
  ...
  defaultTweetReplyToDecorator = function(tweet, options)
	{
    var html = '';
    var tweet_screen_name = null;
    if (tweet.user && tweet.user.screen_name) {
      tweet_screen_name = tweet.user.screen_name;
    } else if (tweet.from_user){
      tweet_screen_name = tweet.from_user;
    }
    if (tweet_screen_name && tweet['id']) {
      var reply_url = 'http://twitter.com/?status=@' + tweet_screen_name + "%20&in_reply_to_status_id=" + tweet['id'] + "&in_reply_to=" + tweet_screen_name;
  		html = '<span class="jta-tweet-replyto">' +
  		' ' +
  		'<a class="jta-tweet-replyto-link" href="' + reply_url + '" target="_blank">reply' +
  		'</a>' +
  		'</span>';
    } 
		return html;
	...
	getFeedUrl = function(options, flPaging)
	{
		...
		if (options.searchParams)
		{
			url += '//' + options.search_api_hostname + '/search.json?' +
				((options.searchParams instanceof Array) ? options.searchParams.join('&') : options.searchParams) +
				'&rpp=100';
		}
		else if (options.list)
		{
			url += '//' + options.list_api_hostname + '/1/' + options.username + '/lists/' + options.list + '/statuses.json?per_page=20';
		}
		else
		{
			url += '//' + options.timeline_api_hostname + '/1/statuses/user_timeline.json?screen_name=' + options.username + '&count=20';
	...
	{
		$.getJSON("http://" + options.ratelimit_api_hostname + "/1/account/rate_limit_status.json?callback=?", function(rateLimit)
		{
